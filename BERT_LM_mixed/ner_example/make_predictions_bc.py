...
def read_pred_input(temp_dir, input_dir, filename, max_l):
    ...
    with open(filepath, 'r', encoding='utf-8') as f1:
        for line in f1:
            line=line.strip()
            if len(line)>(max_l-10):
                seg_line=cut_text(line, (max_l-10))
                for one_seg in seg_line:
                    outcontents.append('\n'.join([x for x in one_seg]))
            else:
...
def main():
    ...
    # parser.add_argument("--except_filelist_path", type=str, default="", help="if given, exclude these files when reading data")
    ...
    if not os.path.exists(args.pred_output_dir):
        ...
        exclude_list=os.listdir(args.pred_output_dir)
    ...
    if exclude_list:
        if args.overwrite_output_dir:
            ...
            allfiles=os.listdir(args.pred_input_dir)
            inputfiles=[x for x in allfiles if x not in exclude_list]  
    else:
        inputfiles=os.listdir(args.pred_input_dir)

    for onefile in inputfiles:
        temp_filename, temp_path=read_pred_input(args.pred_input_dir, args.pred_input_dir, onefile, args.max_seq_length)
        predictions = evaluate(args, model, tokenizer, labels, pad_token_label_id, mode="test", filename=temp_filename)
        
        out_predpath = os.path.join(args.pred_output_dir, onefile.replace('.txt', '.pred'))
        
        with open(out_predpath, "w+", encoding='utf-8') as writer:
            with open(os.path.join(args.pred_input_dir, temp_filename), "r") as f:
                example_id = 0
                for line in f:
                    if not line.strip():
                        writer.write(line)
